...
<%
	...
	List<BlogUser> fansList = null;
	
	fansList = userMan.getFans(name);
%>
<head>
	...
	<title>Babbly - <%=user.getRealname()%>'s Followers</title>	
...
<div id="main">
	<h2 id="sub_tit"><%=user.getRealname()%>'s Followers</h2>
	<div id="user_grid">
	<table><tbody>
	<%
		int i = 1;
		...
		for(BlogUser fans : fansList){
			if(i%2 == 1) { out.println("<tr>"); }
			out.println("<td class=\"data-col\">");
	%>
		<div class="div2">
			<div class="postfig float-left">
				<a style="hide-focus: true" href="myblog.jsp?name=<%=fans.getName()%>" target="_blank" title="<%=fans.getRealname()%>">
					<img class="img_fra" src="<%=fans.getPhotoUrl()%>" onload="shrinkImage(this,48,48);">
				</a> 
			</div>
			<div class="city-info float-left" style="margin-left:10px;">
				<div class="text-left name">
					<a style="hide-focus: true" href="myblog.jsp?name=<%=fans.getName()%>">
						<%=fans.getRealname()%>
					</a>
				</div>
				<div class="sex float-left <%=fans.getGender()==1?"female":"male"%>"></div>
				<div class="float-left city"><%=fans.getAddress().isEmpty()?"未知":fans.getAddress()%></div>
				<div style="float:right; margin-top:-8px; position:relative;">
					<span id="concern-f">
<%		
		if(currUser.getId() != fans.getId()){ //不是本人
			if(!userMan.isFriend(currUser.getId(),fans.getId())) {	//当前用户与fans不是好友关系
%>
					<input type="button" class="button-plane" onclick="addAttention('<%=currUser.getId()%>','<%=fans.getId()%>');" value="添加关注"/>
<%			} else {	 %>
					<input type="button" class="button-plane" onclick="cancelAttention('<%=currUser.getId()%>','<%=fans.getId()%>');" value="取消关注"/>
<%			}	
		}
%>
					</span>
